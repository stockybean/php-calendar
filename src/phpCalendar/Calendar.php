...
namespace stockybean\phpCalendar;
...
class Calendar
{
    ...
    public function draw($date, $color)
    {
        ...
        $past = ( $month < date('m') ) ? 'past' : '';

        $current = ( $month == date('m') ) ? 'current col-lg-12 pb-lg-5 pb-4' : 'col-lg-4';

        $calendar .= '<div class="' . $past . ' ' . $current . '">';

        $calendar .= '<table class="calendar mb-4' . $color . '">';
